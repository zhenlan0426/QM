...
class Net_int_2Edges_Set2Set(torch.nn.Module):
    # use set2set as head
    def __init__(self,dim=64,edge_dim=12,node_in=8,edge_in=19,edge_in3=8):
        super(Net_int_2Edges_Set2Set, self).__init__()
        self.lin_node = torch.nn.Linear(node_in, dim)
        self.lin_edge_attr = torch.nn.Linear(edge_in, edge_dim)
        ...
        self.lin_weight = Linear(8, dim*2*2, bias=False)
        self.lin_bias = Linear(8, 1, bias=False)
        
        self.norm = BatchNorm1d(dim*2*2)
        self.norm_x = BatchNorm1d(node_in)
        self.set2set = Set2Set(dim*2,processing_steps=3,num_layers=3)
...
class Net_int_2Edges_Set2Set2(torch.nn.Module):
    # use set2set as head
    def __init__(self,dim=64,edge_dim=12,node_in=8,edge_in=19,edge_in3=8,num_layers=1):
        super(Net_int_2Edges_Set2Set2, self).__init__()
        self.num_layers = num_layers
        self.dim = dim
        self.lin_node = torch.nn.Linear(node_in, dim)
        self.lin_edge_attr = torch.nn.Linear(edge_in, edge_dim)
        ...
        self.q_star_lin = Linear(edge_in3,dim*2*2)
    ...
    def forward(self, data,IsTrain=False):
        ...
        q_star = self.q_star_lin(data.edge_attr3)
...
class Net_int_2Edges_Set2Set3(torch.nn.Module):
    # use set2set as head, pool->q_star, type->h0
    def __init__(self,dim=64,edge_dim=12,node_in=8,edge_in=19,edge_in3=8,num_layers=1):
